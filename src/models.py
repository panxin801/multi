...
class Model(torch.nn.Module):
    ...
    def forward(self,
        ...
        logits, atten_info = self.get_logits(batch_wave,
                                             ...
                                             target_lengths,
                                             return_atten=True)
        losses = self._compute_cross_entropy_losses(logits, target_labels,
                                                    target_paddings)
        loss = torch.sum(losses)
        if label_smooth > 0:
            loss = loss * (1 - label_smooth) + self._uniform_label_smooth(
                logits, target_paddings) * label_smooth
        if lst_w > 0.:
            loss = loss * (1 - lst_w) + self._lst(
                logits, target_ids, target_paddings, T=lst_t) * lst_w
        if return_atten:
    ...
    def _compute_cross_entropy_losses(self, logits, labels, paddings):
        ...
        losses = F.cross_entropy(
            logits.view(-1, V), labels.view(-1), reduction="none").view(
                B, T) * (1 - paddings).float()
        return losses
    ...
    def get_logits(self,
        ...
        if return_atten:
            ...
            timer.tic()
            encoder_outputs, encoder_output_lengths, enc_self_atten_list = self.encoder(
                sp_outputs, sp_output_lengths, return_atten=True)
            logging.debug("encoder time: {}s".format(timer.toc()))
            timer.tic()
            outputs, decoder_atten_tuple_list = self.decoder(
                encoder_outputs,
                encoder_output_lengths,
                target_ids,
                target_lengths,
            ...
            timer.tic()
            return outputs, (encoder_outputs, encoder_output_lengths,
                             enc_self_atten_list, target_lengths,
                             decoder_atten_tuple_list, sp_outputs,
                             sp_output_lengths)
        else:
            timer = utils.Timer()
            timer.tic()
            encoder_outputs, encoder_output_lengths = self.splayer(
                batch_wave, lengths)
            logging.debug("splayer time: {}s".format(timer.toc()))
            timer.tic()
            encoder_outputs, encoder_output_lengths = self.encoder(
                encoder_outputs, encoder_output_lengths, return_atten=False)
            logging.debug("encoder time: {}s".format(timer.toc()))
            timer.tic()
    ...
    def decode(self,
        ...
        encoder_outputs, encoder_output_lengths = self._get_acoustic_representations(
            batch_wave, lengths)
        target_ids, scores = self._beam_search(encoder_outputs,
                                               encoder_output_lengths,
                                               nbest_keep, sosid, eosid,
                                               maxlen)
    ...
    def _beam_search(self, encoder_outputs, encoder_output_lengths, nbest_keep,
                     sosid, eosid, maxlen):

        B = encoder_outputs.shape[0]
        # init
        init_target_ids = torch.ones(B, 1).to(
            encoder_outputs.device).long() * sosid
        init_target_lengths = torch.ones(B).to(encoder_outputs.device).long()
        outputs = (self.decoder(encoder_outputs, encoder_output_lengths,
                                init_target_ids, init_target_lengths)[:,
                                                                      -1, :])
        ...
        target_ids = torch.ones(B * nbest_keep, 1).to(
            encoder_outputs.device).long() * sosid
        target_lengths = torch.ones(B * nbest_keep).to(
            encoder_outputs.device).long()

        target_ids = torch.cat(
            [target_ids, nbest_ids.view(B * nbest_keep, 1)], dim=-1)
        target_lengths += 1
        ...
        for step in range(1, maxlen):
            (nbest_ids, nbest_logprobs, beam_from) = self._decode_single_step(
                encoder_outputs, encoder_output_lengths, target_ids,
                target_lengths, nbest_logprobs, finished_sel)
            batch_idx = (torch.arange(B) * nbest_keep).view(B, -1).repeat(
                1, nbest_keep).contiguous().to(beam_from.device)
            batch_beam_from = (batch_idx +
                               beam_from.view(-1, nbest_keep)).view(-1)
        ...
        for i in range(len(ended)):
            if ended[i][0] == sosid:
                formated[ended_batch_idx[i]].append(
                    (ended[i], ended_scores[i]))
        for i in range(B):
            formated[i] = sorted(formated[i], key=lambda x: x[1],
                                 reverse=True)[:nbest_keep]

        target_ids = torch.zeros(B, nbest_keep,
                                 maxlen + 1).to(encoder_outputs.device).long()
    ...
    def _decode_single_step(self,
        ...
        if B_d % B != 0:
            raise ValueError(
                "The dim of target_ids does not match the encoder_outputs.")
        nbest_keep = B_d // B
        encoder_outputs = (encoder_outputs.view(B, 1, T_e, D_e).repeat(
            1, nbest_keep, 1, 1).view(B * nbest_keep, T_e, D_e))
        encoder_output_lengths = (encoder_output_lengths.view(B, 1).repeat(
            1, nbest_keep).view(-1))

        # outputs: [B, nbest_keep, vocab_size]
        outputs = (self.decoder(encoder_outputs, encoder_output_lengths,
                                target_ids, target_lengths)[:, -1, :])
        vocab_size = outputs.size(-1)
        outputs = outputs.view(B, nbest_keep, vocab_size)
        log_probs = F.log_softmax(outputs,
                                  dim=-1)  # [B, nbest_keep, vocab_size]
        if finished_sel is not None:
            log_probs = log_probs.view(B * nbest_keep, -1) - finished_sel.view(
                B * nbest_keep, -1).float() * 9e9
            log_probs = log_probs.view(B, nbest_keep, vocab_size)
        this_accumu_scores = accumu_scores.view(B, nbest_keep, 1) + log_probs
        topk_res = torch.topk(this_accumu_scores.view(B,
                                                      nbest_keep * vocab_size),
                              k=nbest_keep,
                              dim=-1)
...
class LM(torch.nn.Module):
    ...
    def forward(self, ids, labels, paddings, label_smooth=0.):
        ...
        if label_smooth > 0:
            tot_loss = tot_loss * (1 -
                                   label_smooth) + self._uniform_label_smooth(
                                       logits, paddings) * label_smooth
    ...
    def fetch_vis_info(self, ids, labels, paddings):
        ...
        elif (isinstance(self.lm_layer, clozer.ClozerV2)
              or isinstance(self.lm_layer, clozer.Clozer)
              or isinstance(self.lm_layer, clozer.UniClozer)
              or isinstance(self.lm_layer, clozer.BwdUniClozer)):
            logits, atten = self.lm_layer(ids, lengths, return_atten=True)
        else:
    ...
    def _compute_ce_loss(self, logits, labels, paddings):
        D = logits.size(-1)
        losses = F.cross_entropy(logits.view(-1, D).contiguous(),
                                 labels.view(-1),
                                 reduction='none')
    ...
    def _compute_ncorrect(self, logits, labels, paddings):
        ...
        n_correct = torch.sum((pred == labels.view(-1)).float() *
                              (1 - paddings).view(-1).float())
        return n_correct
    ...
    def restore(self, pkg):
        ...
        for key in self.lm_layer.config.keys():
            if (key != "dropout_rate"
                    and self.lm_layer.config[key] != pkg["lm_config"][key]):
