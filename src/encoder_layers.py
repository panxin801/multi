...
class Conv1dSubsample(torch.nn.Module):
    ...
    def forward(self, feats, feat_lengths):
        ...
        output_lengths = ((feat_lengths - 1 *
                           (self.context_width - 1) - 1) / self.subsample +
                          1).long()
...
class Conv2dSubsample(torch.nn.Module):
    ...
    def __init__(self, input_dim, d_model):
        ...
        self.conv = torch.nn.Sequential(torch.nn.Conv2d(1, 32, 3, 2),
                                        torch.nn.ReLU(),
                                        torch.nn.Conv2d(32, 32, 3, 2),
                                        torch.nn.ReLU())
        self.affine = torch.nn.Linear(32 * (((input_dim - 1) // 2 - 1) // 2),
                                      d_model)
...
class Conv2dSubsampleV2(torch.nn.Module):
    def __init__(self, input_dim, d_model, layer_num=2):
        ...
        for i in range(layer_num - 1):
            layers += [("subsample/conv{}".format(i + 1),
                        torch.nn.Conv2d(32, 32, 3, (2, 1))),
                       ("subsample/relu{}".format(i + 1), torch.nn.ReLU())]
        layers = OrderedDict(layers)
        self.conv = torch.nn.Sequential(layers)
        self.affine = torch.nn.Linear(32 * (input_dim - 2 * layer_num),
                                      d_model)
...
class Transformer(torch.nn.Module):
    def __init__(self, config):
        ...
        if self.subconf["type"] == "ConvV1":
            ...
            self.sub = Conv2dSubsampleV2(self.input_dim, self.d_model,
                                         self.subconf["layer_num"])
        elif self.subconf["type"] == "Stack":
            self.context_width = config["context_width"]
            self.subsample = config["subsample"]
            self.sub = Conv1dSubsample(self.input_dim, self.d_model,
                                       self.context_width, self.subsample)
