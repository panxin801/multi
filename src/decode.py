...
def get_args():
    ...
    parser.add_argument("--feed-batchsize",
                        type=int,
                        default=20,
                        help="batch_size")
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(args.data_dir, 'wav.scp')):
        ...
        test_loader = torch.utils.data.DataLoader(
            test_set,
            collate_fn=data.kaldi_wav_collate,
            shuffle=False,
            batch_size=args.feed_batchsize)
    elif os.path.exists(os.path.join(args.data_dir, 'feats.scp')):
        offline = True
        test_loader = torch.utils.data.DataLoader(
            test_set,
            collate_fn=data.kaldi_feat_collate,
            shuffle=False,
            batch_size=args.feed_batchsize)
    ...
    for utts, padded_waveforms, wave_lengths in test_loader:
        ...
        if not offline:
            ...
            wave_time += wave_lengths.sum().numpy(
            ) / 100.  # by default, 100 frames cost 1 sec.
        if next(model.parameters()).is_cuda:
            padded_waveforms = padded_waveforms.cuda()
            wave_lengths = wave_lengths.cuda()
        with torch.no_grad():
            target_ids, scores = model.decode(padded_waveforms,
                                              wave_lengths,
                                              nbest_keep=args.nbest,
                                              maxlen=args.maxlen)
